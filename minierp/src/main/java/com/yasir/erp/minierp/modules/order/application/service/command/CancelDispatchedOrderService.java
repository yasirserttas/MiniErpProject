...
import com.yasir.erp.minierp.modules.order.domain.port.inbound.command.CancelDispatchOrderUseCase;
...
@Service
public class CancelDispatchedOrderService implements CancelDispatchOrderUseCase {
    ...
    public CancelDispatchedOrderService(OrderActiveQueryPort orderActiveQueryPort,
    ...
    public OrderDto cancelDispatchOrder(String orderId) {
        ...
        if (OrderStatus.CANCELLED.equals(order.getOrderStatus()) ||
            ...
            throw new IllegalStateException
                    ("Sadece DISPATCHED durumundaki sipariş iptal edilebilir.");
        }
        ...
        DispatchNote dispatchNote = dispatchNoteOrderQueryPort.
                findByOrderIdAndActive(orderId,true)
                .orElseThrow(()-> new OrderNotFoundException(orderId));

        cancelDispatchNote.cancelDispatchNote(dispatchNote.getId());

        Invoice inv = invoiceOrderQueryPort.findByOrderIdAndActive(orderId, true)
                .orElseThrow(()-> new OrderNotFoundException(orderId));
